...
> **Output home:** Store ideation sessions in `.agents/brainstorms/` (root or service-specific) using hash-prefixed filenames like `abcd1234-ideas.md`. Cross-link to the originating task so concepts have traceability.
...
- Update personal cheat sheets in `.agents/notes/brainstormer-mode-cheat-sheet.md` with creative prompts or recurring decision matrices.

## Typical Actions
- Review related `.agents/tasks/`, audits, or feedback items before ideating.
...
- Share the resulting `.agents/brainstorms/<hash>.md` in the relevant task file with a short synopsis.
