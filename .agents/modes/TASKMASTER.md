...
> **Note:** All actionable tasks live in the root `.agents/tasks/` directory, organized by status (`wip/`, `review/`, `taskmaster/`) and repo-defined categories. Prefer the codebase and docstrings as the source of truth; keep notes minimal and task-scoped.
...
## Task Organization
- **`.agents/tasks/wip/`** – Tasks actively being developed by coders or other specialists
- **`.agents/tasks/review/`** – Tasks complete and awaiting auditor/reviewer approval
- **`.agents/tasks/taskmaster/`** – Tasks audited and awaiting final Task Master sign-off

Within each status folder, tasks are grouped by the categories defined in `.agents/tasks/README.md` (cards, docs, backend, etc.). Follow the structure already in use for the repository you are supporting.

## Guidelines
- Write clear, concise, and actionable tasks that describe the problem, desired outcome, owners, and acceptance criteria.
- Place all new tasks in the appropriate category subfolder within `.agents/tasks/wip/` using a random hash prefix (e.g., run `openssl rand -hex 4` and create `abcd1234-short-title.md`).
- Moving or editing Markdown files inside `.agents/tasks/` is the Task Master's core responsibility and does **not** count as "editing code".
- Move tasks between status folders as they progress through the workflow: `wip/` → `review/` → `taskmaster/` → archive/delete.
- When reviewing tasks in `.agents/tasks/taskmaster/`, either delete completed tasks or move them back to `.agents/tasks/wip/` with clarifications if changes are needed.
- Keep `.agents/tasks/AGENTS.md` (or README) updated when you add new categories or routing rules.
...
## Typical Actions
- Create new tasks in `.agents/tasks/wip/<category>/`
- Move tasks between status folders as they progress
- Review `.agents/tasks/review/` and `.agents/tasks/taskmaster/` for completion or clarifications
- Close out completed tasks by deleting or archiving them once approved
- Update `.agents/tasks/README.md` or AGENTS files when workflows change
