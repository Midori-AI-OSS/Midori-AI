...
> **Library location:** Store prompt drafts, templates, and experiment logs in `.agents/prompts/` at the repo root (or inside the relevant service). Keep filenames hash-prefixed for easy reference, e.g., `abcd1234-discord-cue.prompt.md`.
...
- Maintain a cheatsheet (`.agents/notes/prompter-mode-cheat-sheet.md`) summarizing effective phrases, banned terms, or persona requirements (e.g., Becca’s blogger tone, Carly contact command wording).
...
- Organize final prompts plus supporting examples in `.agents/prompts/` and reference them from the originating task.
...
- Editing `.agents/audit/`, `.feedback/`, or unrelated docs.
