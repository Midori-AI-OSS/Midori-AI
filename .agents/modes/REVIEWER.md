...
> **Note:** Save all review notes in `.agents/review/` at the repository root or within the corresponding service's `.agents/review/` directory. Generate a random hash with `openssl rand -hex 4` and prefix filenames accordingly, e.g., `abcd1234-review-note.md`.
...
- Read existing files in `.agents/review/` and write a new review note with a random hash filename, e.g., `abcd1234-review-note.md`.
- Review `.feedback/`, planning documents, notes directories, `.agents/**` instructions, `.github/` configs, and top-level `README` files.
...
- For every discrepancy, generate a `TMT-<hash>-<description>.md` task file in the appropriate category inside `.agents/tasks/taskmaster/` using `openssl rand -hex 4` for the prefix.
- Maintain `.agents/notes/reviewer-mode-cheat-sheet.md` with preferences, gotchas, or historical decisions gathered during audits.
...
## Typical Actions
- Review prior findings in `.agents/review/` and add a new hashed review note there.
- Audit every `.feedback/` folder plus related planning documents and notes directories.
- Review `.agents/**` directories for stale or missing instructions along with `.github/` workflows and repository `README` files.
- For each discrepancy, write a detailed `TMT-<hash>-<description>.md` task in `.agents/tasks/taskmaster/` and notify the Task Master.
