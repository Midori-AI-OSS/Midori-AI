...
- **Blogger Mode** (`.codex/modes/blog/BLOGGER.md`): For creating, publishing, and managing blog posts. Bloggers use shell scripts (see `.codex/blog/scripts/post_blog.sh`) to simulate posting (echo a message). Keep drafts for human posting; before generating a new batch, archive/move old drafts out of the active folder so only the current batch is queued. Bloggers should keep their cheat sheet in `.codex/notes/` up to date.
- **Blog-Prompter Mode** (`.codex/modes/blog/BLOG-PROMPTER.md`): Combines staged gatherer briefs into a single Blogger-ready handoff and deletes the staged source briefs so Blogger only reads one file.
- **Change-Diff-Gatherer Mode** (`.codex/modes/blog/CHANGE-DIFF-GATHERER.md`): Gathers git diff-based updates and stages a concise brief (no raw diffs; no commit IDs in the staged brief).
- **Change-PR-Gatherer Mode** (`.codex/modes/blog/CHANGE-PR-GATHERER.md`): Gathers pull request themes via `gh` and stages a brief (no PR numbers/titles/URLs in the staged brief).
- **Change-Issue-Gatherer Mode** (`.codex/modes/blog/CHANGE-ISSUE-GATHERER.md`): Gathers issue themes via `gh` and stages a brief (no issue numbers/titles/URLs in the staged brief).
- **Change-Context-Gatherer Mode** (`.codex/modes/blog/CHANGE-CONTEXT-GATHERER.md`): Gathers local workflow/context signals (tasks/prompts) and stages a short context brief (no speculation).
