...
- **Task Master Mode** (`.agents/modes/TASKMASTER.md`): For creating, organizing, and maintaining actionable tasks in the root `.agents/tasks/` folder. Task Masters define and prioritize work for Coders and ensure tasks are ready for implementation.
- **Manager Mode** (`.agents/modes/MANAGER.md`): For planning, coordination, and maintaining contributor guidance across the monorepo. Managers steward `AGENTS.md`, mode docs, and process alignment with Task Masters and stakeholders.
- **Coder Mode** (`.agents/modes/CODER.md`): For implementing, refactoring, and reviewing code. Focuses on high-quality, maintainable, and well-documented contributions. Coders regularly review the `.agents/tasks/` folder for new or assigned work.
- **Reviewer Mode** (`.agents/modes/REVIEWER.md`): For auditing repository documentation and filing `TMT`-prefixed tasks when updates are needed.
- **Auditor Mode** (`.agents/modes/AUDITOR.md`): For performing comprehensive reviews and audits. Emphasizes thoroughness, completeness, and catching anything others may have missed.
- **Blogger Mode** (`.agents/modes/blog/BLOGGER.md`): For creating, publishing, and managing blog posts. Bloggers use shell scripts (see `.agents/blog/scripts/post_blog.sh`) to simulate posting (echo a message). Keep drafts for human posting; before generating a new batch, archive/move old drafts out of the active folder so only the current batch is queued. Bloggers should keep their cheat sheet in `.agents/notes/` up to date.
- **Blog-Prompter Mode** (`.agents/modes/blog/BLOG-PROMPTER.md`): Combines staged gatherer briefs into a single Blogger-ready handoff and deletes the staged source briefs so Blogger only reads one file.
- **Change-Diff-Gatherer Mode** (`.agents/modes/blog/CHANGE-DIFF-GATHERER.md`): Gathers git diff-based updates and stages a concise brief (no raw diffs; no commit IDs in the staged brief).
- **Change-PR-Gatherer Mode** (`.agents/modes/blog/CHANGE-PR-GATHERER.md`): Gathers pull request themes via `gh` and stages a brief (no PR numbers/titles/URLs in the staged brief).
- **Change-Issue-Gatherer Mode** (`.agents/modes/blog/CHANGE-ISSUE-GATHERER.md`): Gathers issue themes via `gh` and stages a brief (no issue numbers/titles/URLs in the staged brief).
- **Change-Context-Gatherer Mode** (`.agents/modes/blog/CHANGE-CONTEXT-GATHERER.md`): Gathers local workflow/context signals (tasks/prompts) and stages a short context brief (no speculation).
- **Brainstormer Mode** (`.agents/modes/BRAINSTORMER.md`): For collaborative idea generation and capturing options for future tasks.
- **Prompter Mode** (`.agents/modes/PROMPTER.md`): For crafting high-quality prompts for LLM/LRM interactions.
- **Unknown Mode** (no file): If you are unsure which mode applies or are asked to perform an action outside your capabilities—such as creating a task while in read-only mode—review the guides in `.agents/modes/`, choose the closest fit, and use the team communication command (`contact.sh`) to clarify the situation. This helps us improve our prompting and documentation for future contributors.

Refer to the relevant mode guide in `.agents/modes/` before starting work. For service-specific details, read the service's own `AGENTS.md` and follow existing in-repo guidance.
